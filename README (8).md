...
* Accepts the target IP or hostname, SSH username, SSH key or password details, sudo password when needed, server metadata, container image, API endpoint, and the `root.crt` file from command-line flags, one-at-a-time interactive terminal prompts, or GUI fields.
* Copies `root.crt` to the target certificate directory.
* Creates the remote `docker-compose.yml` with the values entered in the command line, interactive terminal, or GUI.
...
* Logs every step to stdout in CLI/interactive mode, to the GUI in GUI mode, and always appends the same log to `~/.server-data-collector-deployer.log` on the machine running the deployer.
...
### Run interactively on a headless server

If you do not want to provide a long command, start interactive mode and answer each prompt one at a time:

```bash
python3 deploy_server_data_collector_gui.py --interactive
```

The interactive shell shows defaults in brackets. Press **Enter** to accept a default, or type a new value. It prompts for required values until they are filled, hides SSH/sudo password entry, and asks at the end whether to validate only or perform the full deployment.

On a headless system with no display, running the script with no arguments also starts the interactive terminal prompts automatically:

```bash
python3 deploy_server_data_collector_gui.py
```

You can still pre-fill any value and let interactive mode ask for the rest:
...
* `--interactive` starts the one-question-at-a-time terminal prompt flow.
